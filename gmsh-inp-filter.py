...
# Last modified: 2024-02-24T18:58:03+0100
...
If you want to generate hex elements, have the following options set in your
geo-file:
...
def main(args):
    ...
    # “nodes” is a dict mapping node numbers to (x,y,z)-tuples.
    logging.info(f"nodes: {len(nodes)}.")
    Eall, volsets, nreverse, setnodes, veltype = retrieve_C3Dx(headings, lines)
    # “Eall”  maps element numbers to lists of node numbers.
    # “volsets” maps volume set names to lists of element numbers.
    # “nreverse” maps node numbers to lists of element numbers.
    # “setnodes” maps volume set names to sets of node numbers.
    # “veltype” denotes the type of volume elements found.
    ...
    surfsets, surfnodes, seltype = retrieve_CPSx(headings, lines)
    ...
    # “surfsets” maps names to surface sets.
    # A surface set maps element numbers to n-tuples of node numbers.
    # "surfnodes" maps surface set names to sets of node numbers.
    ...
    for sname, sv in surfsets.items():
        ...
        surfaces[sname] = remap_surface(sv, Eall, seltype)
    # Get the node numbers used in Eall.
    Nall = set()
    for v in Eall.values():
        Nall.update(v)
    Nall = tuple(sorted(Nall))
    if max(Nall) - min(Nall) + 1 == len(Nall):
...
def retrieve_C3Dx(headings, lines):
    """
    Extract C3D10 and C3D20 element sets from the file contents.
    ...
    Returns:
        ...
        5) String containing the type of element found; C3D10 or C2D20.
    """
    all_elements = {}
    element_sets = {}
    all_nreverse = {}
    ...
    for h in headings:
        ...
        if not lname.startswith("element"):
            continue
        if "type=C3D20".lower() in lname:
            eltype = "C3D20"
        elif "type=C3D10".lower() in lname:
            eltype = "C3D10"
        else:
            continue
    ...
    return all_elements, element_sets, all_nreverse, all_setnodes, eltype


def retrieve_CPSx(headings, lines):
    """
    Extract CPS6 or CPS8 element sets from the file contents.
    ...
    Returns:
        1) A dict mapping set names to lists of element numbers.
        2) A dict mapping set names to lists of node numbers.
        3) A string indicating the type of element found.
    """
    element_sets = {}
    node_sets = {}
    eltype = None
    ...
    for h in headings:
        ...
        if not lname.startswith("element"):
            continue
        if "type=CPS8".lower() in lname:
            eltype = "CPS8"
        elif "type=CPS6".lower() in lname:
            eltype = "CPS6"
        else:
            continue
        setname = [s.strip()[6:] for s in name.split(",") if "elset=" in s.lower()]
    ...
    return element_sets, node_sets, eltype


def read_elements(lines, start, end, setname, minnodes=6):
    """
    Extract elements.

    Arguments:
        lines: A list of lines.
        start: first line to process.
        end: one past the last line to process.
        setname: name of the element set.
        minnodes: minumum number of nodes in an element.
    ...
    consol = list(comb(part))

    logging.debug(f"consolidated {len(consol)} lines in set “{setname}”")
    elements = {}
    reverse = {}
    nodes = set()
    for ln in consol:
        elnum, *nodenums = [int(i) for i in ln.split(",")]
        # Sometimes the generated mesh will contain some first order elements.
        # Ignore those.
        if len(nodenums) < minnodes:
            logging.warning(f"element number {elnum}; only {len(nodenums)} nodes; skipped")
            continue
...
def fix_volume_set_names(lines, headings, volsets, nodes):
    ...
    Arguments:
        lines: A list of lines.
        headings: A list of (name, start, end) tuples.
        volsets: A dict mapping volume names to lists of element numbers.
        nodes: A dict mapping set names to sets of node numbers.

    Returns:
        1) A dict mapping volume set names to lists of element numbers.
        2) A dict mapping volume set names to lists of node numbers.
    """
    newsets = {}
    newnodes = {}
    for h in headings:
        ...
        for oname in volsets.keys():
            if set(volsets[oname].keys()).issubset(set(elset)):
                if setname not in newsets:
                    ...
                    logging.info(f"renamed set “{oname}” to {setname}")
                else:
                    newsets[setname].update(volsets[oname])
                    newnodes[setname].update(nodes[oname])
                    logging.info(f"added set “{oname}” to “{setname}”")
    for st in newsets:
...
def fix_surface_set_names(lines, headings, surfsets, surfnodes):
    ...
    Arguments:
        lines: A list of lines.
        headings: list of (name, start, end) tuples.
        surfsets: A dict mapping surface set names to lists of element numbers.
        surfnodes: A dict mapping set names to lists of nodes.
...
def remap_surface(surface_set, Eall, seltype):
    """
    Map surface elements to sides of solid elements.

    Arguments:
        surface_set: A dictionary of surface elements mapping
            element numbers to lists of node numbers.
        Eall: A dictionary of volume elements mapping element numbers
            and containing lists of node numbers.
        seltype: Type of surface element to remap; "CPS6" or "CPS8"
...
def make_eqns(nodes, Eall, nsets, volsets, nreverse, surfsets):
    ...
    Arguments:
        nodes: a dict mapping node numbers to (z,y,z)-tuples.
        Eall: a dict mapping element numbers to lists of node numbers.
        nsets: a dict mapping node set names to sets of node numbers.
        volsets: a dict mapping volume set name to lists of element numbers.
        nreverse: a dict mapping node numbers to lists of element numbers.
        surfsets: a dict mapping set names to sets of node numbers.

    Returns:
        1) A list of *EQUATION strings.
        2) Updated nodes dict.
        3) Updated Eall dict.
        4) Updated surfsets dict.
    """
    # Make a shallow copy of dicts to prevent unintended side effects.
    nodes = nodes.copy()
    Eall = Eall.copy()
    surfsets = surfsets.copy()
        ...
        for setname in others:
            ...
            for ssn, ssv in surfsets.items():
                if node in ssv:
                    ssv.add(newnum)
                    add = False
                    logging.debug(f"added node {newnum} to surface set “{ssn}”")
            # create equation if a node is *not* in a surfset.
            if add:
                for j in (1, 2, 3):
                    # Note: newnum *must* come first.
                    equations.append(f"*EQUATION\n2\n{newnum},{j},-1.0,{node},{j},1.0")
            # last thing to do
            newnum += 1
...
def volume_sets_by_node(setnodes):
    ...
    Arguments:
        setnodes: a dict mapping set names to sets of node numbers.

    Returns:
        A dict mapping node numbers to of sets of element set names.
    """
    im = {}
...
def write_output(
    outfn, nodes, elements, nsets, surfsets, elsets, surfaces, reduced, equations, eltype
):
    with open(outfn, "w") as outf:
        logging.info(f"opened output file “{outfn}”.")
        write_nodes(outf, nodes)
        write_elements(outf, elements, reduced, eltype)
...
def write_elements(outf, elements, reduced, eltype):
    extra = ""
    if eltype.upper() == "C3D10":
        outf.write("*ELEMENT, TYPE=C3D10, ELSET=Eall\n")
    elif eltype.upper() == "C3D20":
        if reduced:
...
def write_equations(outf, equations):
    ...
    for eqn in equations:
        outf.write(eqn)
        outf.write("\n")
    logging.info(f"wrote {3*len(equations)} equations.")


def chunked(iterable, n):
    """
    Split iterable up in chunks of length n.
