...
# Last modified: 2020-09-27T15:38:38+0200
...
def main(args):
    ...
    if opts.equations:
        ...
        equations, nodes, Eall = make_eqns(nodes, Eall, setnodes, volsets, nreverse)
    # Write output file
    write_output(
...
def make_eqns(nodes, Eall, nsets, volsets, nreverse):
    ...
    for node, (first, *others) in vs.items():
        ...
        for setname in others:
            ...
            logging.debug(f"replaced node {node} with {newnum} in element {elnum} in set “{setname}”.")
            # Create equation
            for j in (1, 2, 3):
