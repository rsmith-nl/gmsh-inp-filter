...
# Last modified: 2020-09-27T00:24:05+0200
...
def main(args):
    ...
    Eall, volsets, nreverse = retrieve_elements(headings, lines, "C3D20")
    ...
    # “ereverse” is a dict of element set names indexed by node number
    logging.info(f"original solid element sets: {len(volsets)};")
    volsets, revmap = fix_set_names(lines, headings, volsets, "volume")
    _, surfsets, _ = retrieve_elements(headings, lines, "CPS8")
    logging.info(f"original surface element sets: {len(surfsets)};")
    surfsets, _ = fix_set_names(lines, headings, surfsets, "surface")
    ...
    # “nsets” is a dictionary of tuples of node numbers indexed by the set name.
    ...
    if min(Nall) == 1 and max(Nall) == len(Nall):
        ...
        pass
    if opts.equations:
        sk, nk = set(surfsets.keys()), set(nsets.keys())
        sn = sk & nk
        # Nodes in the sn sets should not be in equations.
        logging.info("finding equations.")
        equations, nodes, Eall = make_eqns(nodes, Eall, nsets, volsets, sn, nreverse)
    # Write output file
    write_output(
        opts.outfn, nodes, Eall, nsets, volsets, surfaces, opts.reduced, equations
    )
...
def retrieve_elements(headings, lines, eltype):
    ...
    Returns:
        ...
        4) A dict of element set names indexed by node number
    """
    all_elements = {}
    all_reverse = {}
    element_sets = {}
    ...
    for h in headings:
        ...
        if not setname:
            ...
            setname = setname[0]
        elements, reverse = read_elements(lines, h[1], h[2], setname)
        ...
        for k, v in reverse.items():
            if k in all_reverse:
                ...
                all_reverse[k] = v
    return all_elements, element_sets, all_reverse


def read_elements(lines, start, end, setname):
    """
    Extract elements.
    Returns dict of n-tuple of node numbers, indexed by the element number.
...
def fix_set_names(lines, headings, element_sets, stype):  # noqa
    ...
    for h in headings:
        ...
        try:
            ...
            logging.error(f"{stype} set “{name}” without name")
            continue
        if setname == "all":
            continue
        elset = []
...
def make_eqns(nodes, Eall, nsets, volsets, exclude_from_eqns, nreverse):
    ...
    for n in nodes.keys():
        ...
        for nn in active_sets:
            # There *must* be a node set for every volume set.
            if n in nsets[nn]:
                setnames.append(nn)
        if len(setnames) > 1:
            byvolume[n] = setnames
    ...
    equations = []
    return equations, nodes+newnodes, Eall, nsets
